...
namespace EmployeeManage.Controllers
{
    public class BarcodeController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> BarcodeIndex(BarcodeRequest model, CancellationToken token = default)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    await _iBarcodeRepo.GenerateBarcode(model, token);
                    TempData["success"] = "Added Successfylly";
                    return RedirectToAction("BarcodeIndex", "Barcode");
                }
                var ItemsList = await _iBarcodeRepo.ItemsList(token);
                BarcodeRequest Items = new BarcodeRequest()
                {
        ...
        public async Task<IActionResult> EditBarcode(int id, CancellationToken token = default)
        {
            ...
            var list = await _iBarcodeRepo.ItemsList(token);
            if (result == null)
            {
