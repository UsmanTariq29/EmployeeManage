...
namespace EmployeeManage.Repository
{
    public class BarcodeRepo : IBarcodeRepo
    {
        ...
        private async Task<List<TblItem>> getItems(CancellationToken token = default)
        {
        ...
        public async Task GenerateBarcode(BarcodeRequest model, CancellationToken token = default)
        {
            await ValidateRecord(model.ItemId, token);
            await ValidateBarcodeRecord(model.ItemId, token);
            var barcode = new TblItemBarcode
            {
                ItemId = model.ItemId,
                ItemBarcode = 12345 + model.ItemId.ToString()
            };
